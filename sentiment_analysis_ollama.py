...
INPUT_CSV       = "customer_messages.csv"          # your input file
OUTPUT_JSON     = "sentiment_report.json"           # structured report
OUTPUT_CSV      = "messages_classified.csv"         # classified rows

OLLAMA_MODEL    = "llama3.2:3b"                     # change to "mistral", etc.
...
def deduplicate_messages(rows: list[dict]) -> tuple[list[str], dict[str, int]]:
    ...
    Why this matters:
        If your inbox has 5,000 rows but only 32 unique message templates,
...
def build_report(classified:     list[dict],
    ...
    # High-urgency messages — fuzzy-deduplicated before capping at 20.
    #
